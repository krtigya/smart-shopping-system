...
$shopParams = $_GET;
$shopParams['catalog'] = 1;
header('Location: ' . BASE_URL . '/index.php?' . http_build_query($shopParams));
exit;
...
<div class="min-h-screen flex items-center justify-center relative bg-indigo-50 py-20">
    <div class="relative bg-white rounded-2xl border border-indigo-200 shadow-xl max-w-lg w-full mx-4 p-8 text-center card-hover">
        ...
        <div class="flex justify-center gap-3 mb-8">
            <a href="<?= BASE_URL ?>/index.php" class="px-5 py-2.5 bg-indigo-700 hover:bg-indigo-800 text-white rounded-xl font-medium transition shadow-md hover:shadow-lg">
                Go Home
            </a>
        ...
        <div class="relative mb-6">
            <input type="text" id="search-notfound" placeholder="Search products..."
                   class="w-full pl-4 pr-10 py-2.5 border border-indigo-200 rounded-xl focus:ring-2 focus:ring-indigo-600 focus:border-transparent text-sm">
            <button type="button" onclick="searchRedirect()"
                    class="absolute right-2 top-1/2 -translate-y-1/2 p-2 text-slate-500 hover:text-indigo-700 transition">
