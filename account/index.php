...
<div class="max-w-5xl mx-auto px-4 py-10">
    ...
    <div class="bg-white rounded-3xl border border-indigo-200 shadow-sm p-8 flex flex-col items-center text-center">
        <div class="w-24 h-24 rounded-full bg-gradient-to-br from-indigo-700 to-indigo-800 text-white flex items-center justify-center text-3xl font-bold shadow">
        ...
        <span class="mt-2 inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium             <?= ($user['role'] === 'admin') ? 'bg-indigo-50 text-indigo-700' : 'bg-slate-100 text-slate-600' ?>">
            <?= htmlspecialchars($user['role'] ?? 'user') ?>
        </span>
        <div class="mt-5 flex gap-3">
            <a href="<?= BASE_URL ?>/cart/address.php" class="px-5 py-2.5 rounded-xl bg-indigo-700 hover:bg-indigo-800 text-white text-sm font-semibold shadow transition">Add Address</a>
            <a href="<?= BASE_URL ?>/orders/index.php" class="px-5 py-2.5 rounded-xl border border-indigo-200 text-slate-700 text-sm font-medium hover:bg-indigo-100 transition">My Orders</a>
            <?php if ($user['role'] === 'admin'): ?>
                <a href="<?= BASE_URL ?>/admin/dashboard.php" class="px-5 py-2.5 rounded-xl border border-indigo-200 text-slate-700 text-sm font-medium hover:bg-indigo-100 transition">Admin Panel</a>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
        ...
        <div class="bg-white rounded-3xl border border-indigo-200 shadow-sm p-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="<?= BASE_URL ?>/cart/address.php" class="text-sm font-medium text-indigo-700 hover:text-indigo-800">+ Add</a>
            </div>
        ...
        <div class="bg-white rounded-3xl border border-indigo-200 shadow-sm p-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="<?= BASE_URL ?>/orders/index.php" class="text-sm font-medium text-indigo-700 hover:text-indigo-800">View all</a>
            </div>
