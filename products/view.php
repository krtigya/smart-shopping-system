<?php
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <a href="<?= BASE_URL ?>/index.php" class="text-sm text-indigo-600 hover:text-indigo-800 mb-6 inline-flex items-center gap-2 transition">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-10 bg-white rounded-3xl border border-indigo-200 shadow-lg p-6 sm:p-8">
            ...
            <div class="flex flex-col">
                <span class="text-xs font-semibold uppercase tracking-wider text-indigo-600"><?= htmlspecialchars($product['brand'] ?? '') ?></span>
                <h1 class="text-3xl font-bold mt-1 text-slate-900"><?= htmlspecialchars($product['name']) ?></h1>
                <p class="text-sm text-slate-500 mt-1">Category: <?= htmlspecialchars($product['category'] ?? '—') ?></p>

                <p class="text-2xl font-bold text-slate-900 mt-5">Rs. <span id="priceTag"><?= number_format($base['price'], 2) ?></span></p>

                <dl class="mt-4 space-y-2 text-sm">
                    <div class="flex gap-2"><dt class="text-indigo-600 w-20">Size:</dt><dd id="sizeTag" class="text-slate-700 font-medium"><?= htmlspecialchars($base['size'] ?: '—') ?></dd></div>
                    <div class="flex gap-2"><dt class="text-indigo-600 w-20">UOM:</dt><dd id="uomTag" class="text-slate-700 font-medium"><?= htmlspecialchars($base['uom'] ?: '—') ?></dd></div>
                    <div class="flex gap-2"><dt class="text-indigo-600 w-20">SKU:</dt><dd id="skuTag" class="text-slate-700 font-medium"><?= htmlspecialchars($base['sku'] ?: '—') ?></dd></div>
                </dl>
                ...
                <form method="POST" class="mt-6 flex items-end gap-3">
                    <div>
                        ...
                        <input type="number" name="quantity" id="qty" min="1" max="<?= max(1, $base['stock']) ?>" value="1" class="w-24 border border-indigo-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    </div>
                    <input type="hidden" name="variant_id" id="variantId" value="<?= $base['vid'] ?>">
                    <button type="submit" name="add_to_cart" class="flex-1 bg-indigo-600 hover:bg-indigo-600 text-white font-semibold py-3 rounded-xl transition shadow-md hover:shadow-lg">Add to Cart</button>
