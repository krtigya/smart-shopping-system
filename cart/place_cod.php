...
try {
    foreach ($items as $it) {
        ...
        // Decrement stock
        if ($vid > 0) {
            if (!$conn->query("UPDATE product_variants SET quantity = quantity - $qty WHERE id = $vid AND quantity >= $qty") || $conn->affected_rows !== 1) {
                throw new RuntimeException("Insufficient variant stock.");
            }
        }
        if (!$conn->query("UPDATE products SET quantity = quantity - $qty WHERE id = $pid AND quantity >= $qty") || $conn->affected_rows !== 1) {
...
<div class="min-h-screen flex items-center justify-center px-4">
    <div class="bg-white rounded-2xl border border-indigo-200 shadow-sm max-w-md w-full p-8 text-center">
        ...
        <h1 class="text-2xl font-bold mb-2 text-slate-800">Order Placed!</h1>
        ...
        <div class="flex justify-center gap-3">
             <a href="<?= BASE_URL ?>/orders/index.php" class="px-5 py-2.5 bg-indigo-700 hover:bg-indigo-800 text-white rounded-xl font-medium transition">View My Orders</a>
