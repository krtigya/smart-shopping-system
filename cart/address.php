...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $returnQuery = $_GET;
    unset($returnQuery['from']);
    if (($_GET['from'] ?? '') === 'checkout') {
        header('Location: ' . BASE_URL . '/cart/checkout.php' . ($returnQuery ? ('?' . http_build_query($returnQuery)) : ''));
    } elseif (isset($_GET['product_id'])) {
        header('Location: ' . BASE_URL . '/cart/checkout.php?product_id=' . (int)$_GET['product_id']);
    } else {
...
<div class="max-w-lg mx-auto px-4 py-10">
    <div class="bg-white rounded-2xl border border-indigo-200 shadow-sm p-8">
        <h1 class="text-2xl font-bold text-slate-800"><?= $addr ? 'Edit' : 'Add' ?> Address</h1>
        <p class="text-sm text-slate-500 mt-1">Tell us where to deliver your order.</p>

        <form method="POST" action="<?= BASE_URL ?>/cart/address.php?<?= htmlspecialchars(http_build_query($_GET)) ?>" class="mt-6 space-y-5">
            <div>
            ...
            <div class="flex gap-3 pt-2">
                ...
                <button type="submit" class="flex-1 px-5 py-2.5 rounded-xl bg-indigo-700 hover:bg-indigo-800 text-white text-sm font-semibold shadow transition">Save Address</button>
            </div>
        </form>
    </div>
</div>
