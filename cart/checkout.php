...
if ($addrRes->num_rows === 0) {
    $ret = isset($_GET['product_id']) ? ('?product_id=' . (int)$_GET['product_id']) : '';
    header('Location: ' . BASE_URL . '/cart/address.php' . $ret);
    exit;
}
...
<div class="max-w-3xl mx-auto px-4 py-10">
    ...
    <h1 class="text-3xl font-bold mb-6 text-slate-900">Checkout</h1>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="md:col-span-2 bg-white rounded-2xl border border-indigo-200 shadow-sm p-6">
            ...
            <div class="space-y-4">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex-1">
                        <a href="<?= BASE_URL ?>/products/view.php?id=<?= $it['id'] ?? $it['pid'] ?>" class="font-medium text-slate-900 hover:text-indigo-600 transition"><?= htmlspecialchars($it['name']) ?></a>
                        ...
                        <?php endif; ?>
                        <p class="text-sm text-indigo-600 mt-1">Rs. <?= number_format($it['price'], 2) ?> &times; <?= $it['qty'] ?></p>
                    </div>
                </div>
                <?php endforeach; ?>
            </div>
        ...
        <div class="bg-white rounded-2xl border border-indigo-200 shadow-sm p-6">
            <h2 class="font-semibold text-slate-900 mb-2">Shipping To</h2>
            ...
            <a href="<?= BASE_URL ?>/cart/address.php" class="text-sm font-medium text-indigo-600 hover:text-indigo-400 inline-block mt-2">Change</a>
        </div>
    </div>

    <!-- Payment -->
    <div class="mt-6 bg-white rounded-2xl border border-indigo-200 shadow-sm p-6">
        <h2 class="font-semibold text-slate-900 mb-4">Payment Method</h2>
...
<div id="codModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
    <div class="bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
        <div class="flex items-center gap-3 mb-4">
            ...
            <h3 class="text-lg font-semibold text-slate-900">Confirm Cash on Delivery</h3>
        </div>
