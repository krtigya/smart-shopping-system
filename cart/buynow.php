...
if ($product_result && mysqli_num_rows($product_result) > 0) {
    ...
    if ($current_quantity <= 0) {
        ...
        <div class="min-h-screen flex items-center justify-center px-4">
            <div class="bg-white rounded-2xl border border-indigo-200 shadow-sm max-w-md w-full p-8 text-center">
                ...
                <h1 class="text-2xl font-bold mb-2 text-slate-800">Product Out of Stock!</h1>
                <p class="text-slate-600 mb-6">We're sorry, this product is no longer available.</p>
                 <a href="<?= BASE_URL ?>/index.php" class="inline-block px-5 py-2.5 bg-indigo-700 hover:bg-indigo-800 text-white rounded-xl font-medium transition">Continue Shopping</a>
            </div>
        </div>
        ...
        if (mysqli_query($conn, $order_query)) {
            ...
            if (mysqli_query($conn, "UPDATE products SET quantity = quantity - 1 WHERE id = $product_id AND quantity > 0") && mysqli_affected_rows($conn) === 1) {
                if ($variant_id > 0) {
                    if (!mysqli_query($conn, "UPDATE product_variants SET quantity = quantity - 1 WHERE id = $variant_id AND quantity > 0") || mysqli_affected_rows($conn) !== 1) {
                        die("Insufficient variant stock.");
                    }
                }
                ...
                if (mysqli_query($conn, $delete_cart_query)) {
                    ...
                    <div class="min-h-screen flex items-center justify-center px-4">
                        <div class="bg-white rounded-2xl border border-indigo-200 shadow-sm max-w-md w-full p-8 text-center">
                            ...
                            <h1 class="text-2xl font-bold mb-2 text-slate-800">Order Placed Successfully!</h1>
                            ...
                            <p class="text-slate-600 mb-6">Payment Status: <span class="font-semibold">Pending</span></p>
                             <a href="<?= BASE_URL ?>/orders/index.php" class="inline-block px-5 py-2.5 bg-indigo-700 hover:bg-indigo-800 text-white rounded-xl font-medium transition mr-2">View My Orders</a>
    ...
    <div class="min-h-screen flex items-center justify-center px-4">
        <div class="bg-white rounded-2xl border border-indigo-200 shadow-sm max-w-md w-full p-8 text-center">
            ...
            <h1 class="text-2xl font-bold mb-2 text-slate-800">Product Not Found!</h1>
