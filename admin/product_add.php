        ...
        if ($message === '' && count($imageData) > 0) {
            ...
            if ($stmt->execute()) {
                ...
                foreach ($imageData as $sort => $d) {
                    ...
                    $varStmt->bind_param("iiessdi", $product_id, $image_id, $d['size'], $d['uom'], $skuVal, $d['price'], $d['qty']);
                    $varStmt->execute();
                }
...
<form method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
                ...
                <input name="name" required value="<?= htmlspecialchars($_POST['name'] ?? '') ?>"
                    class="w-full border border-slate-300 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="description" rows="3" class="w-full border border-slate-300 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500"><?= htmlspecialchars($_POST['description'] ?? '') ?></textarea>
            </div>
        </div>
    </div>
    ...
    <div class="flex justify-end gap-3">
        ...
        <button type="submit" class="px-6 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-600 text-white text-sm font-semibold shadow">Save Product</button>
    </div>
</form>
    ...
    <div class="row border border-slate-200 rounded-lg p-4 bg-slate-50">
        ...
        <div class="grid grid-cols-1 md:grid-cols-6 gap-3 items-end">
            <div class="md:col-span-2">
                ...
                <input type="file" name="images[]" accept="image/*" class="block w-full text-sm text-slate-500 file:mr-3 file:py-2 file:px-3 file:rounded-lg file:border-0 file:bg-indigo-50 file:text-indigo-400 file:text-sm file:font-medium hover:file:bg-indigo-100">
            </div>
            <div class="md:col-span-2">
                <label class="block text-xs font-medium text-slate-600 mb-1">Image Description</label>
                <input name="img_desc[]" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500">
            </div>
            <div>
                <label class="block text-xs font-medium text-slate-600 mb-1">Size</label>
                <input name="size[]" placeholder="e.g. M" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500">
            </div>
            <div>
                <label class="block text-xs font-medium text-slate-600 mb-1">UOM</label>
                <input name="uom[]" placeholder="e.g. pcs" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500">
            </div>
            <div>
                <label class="block text-xs font-medium text-slate-600 mb-1">SKU</label>
                <input name="sku[]" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500">
            </div>
            <div>
                <label class="block text-xs font-medium text-slate-600 mb-1">Price (Rs.)</label>
                <input type="number" step="0.01" min="0" name="price[]" required class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500">
            </div>
            <div>
                <label class="block text-xs font-medium text-slate-600 mb-1">Stock Qty</label>
                <input type="number" min="0" name="qty[]" value="0" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500">
            </div>
        </div>
    </div>
</template>
