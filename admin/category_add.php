...
<form method="POST" class="bg-white rounded-xl border border-slate-200 shadow-sm p-6 max-w-2xl space-y-5">
    ...
    <div>
        ...
        <textarea name="description" rows="4" class="w-full border border-slate-300 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-pink-500"><?= htmlspecialchars($category['description'] ?? ($_POST['description'] ?? '')) ?></textarea>
    </div>
    <label class="inline-flex items-center gap-2 text-sm text-slate-700">
        <input type="checkbox" name="status" value="1" <?= (!isset($category) || ($category['status'] ?? 1)) ? 'checked' : '' ?> class="rounded border-slate-300 text-pink-500">
        Active
    </label>
    <div class="flex justify-end gap-3 pt-2">
        ...
        <button type="submit" class="px-6 py-2.5 rounded-xl bg-pink-500 hover:bg-pink-600 text-white text-sm font-semibold shadow">Save Category</button>
    </div>
</form>
